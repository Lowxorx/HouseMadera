    ...
    public class IsolantDAL : DAL, IDAL<Isolant>
    {
        ...
        public int InsertModele(Isolant modele)
        {
            ...
            try
            {
                ...
                int typeIsolantId;
                if (!Synchronisation<ClientDAL, Client>.CorrespondanceModeleId.TryGetValue(modele.TypeIsolant.Id, out typeIsolantId))
                {
                    //si aucune clé existe avec l'id passé en paramètre alors on recherche par valeur
                    typeIsolantId = Synchronisation<ClientDAL, Client>.CorrespondanceModeleId.FirstOrDefault(c => c.Value == modele.TypeIsolant.Id).Key;
